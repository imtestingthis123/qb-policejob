...
local function IsTargetDown(playerId)
    local retval = false
    local hasReturned = false
    QBCore.Functions.TriggerCallback('police:server:isPlayerDown', function(result)
        retval = result
        hasReturned = true
    end, playerId)
    while not hasReturned do
...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        local playerId = GetPlayerServerId(player)
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDown(playerId) then
